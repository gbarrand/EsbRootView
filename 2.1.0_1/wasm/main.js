...
(function (app_main) {
  ...
  function Program(container, canvas) {
    ...
    Module.app_do_works();
    window.requestAnimationFrame(() => Module.app_win_render());
  ...
  Program.prototype._anim_proc = function () {
    if(Module.app_do_works()==1) window.requestAnimationFrame(() => Module.app_win_render());
  }

  Program.prototype._check_anim = function () {
    if(Module.app_have_to_enable_anim()==1) {
      Module.app_reset_have_to_enable_anim();
      g_timer_id = setInterval(this._anim_proc,16);  // 1/60 seconds.
  ...
  Program.prototype._handle_button_down = function(event) {
    if(event.button==0) {
      ...
      var to_render = Module.app_button_down(event.button,event.offsetX,event.offsetY);       
      if(Module.app_do_works()==1) to_render = 1;
      if(to_render==1) window.requestAnimationFrame(() => Module.app_win_render());
      this._check_anim();
    }
  }
  Program.prototype._handle_button_up = function(event) {
    if(event.button==0) {
      ...
      event.stopPropagation();
      event.preventDefault();
      var to_render = Module.app_button_up(event.button,event.offsetX,event.offsetY);
      if(Module.app_do_works()==1) to_render = 1;
      if(to_render==1) window.requestAnimationFrame(() => Module.app_win_render());
      this._check_anim();
    }
  ...
  Program.prototype._handle_button_move = function(event) {
    if( (event.button==0) && (g_mouse_down==1)) {
      event.stopPropagation();
      event.preventDefault();
      var to_render = Module.app_button_move(event.button,event.offsetX,event.offsetY);       
      if(Module.app_do_works()==1) to_render = 1;
      if(to_render==1) window.requestAnimationFrame(() => Module.app_win_render());
      this._check_anim();
    }
